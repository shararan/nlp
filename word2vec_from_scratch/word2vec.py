...
def naiveSoftmaxLossAndGradient(
    ...
    simvec = outsideVectors@centerWordVec # similarity vector
    probdist = softmax(simvec)[:, np.newaxis] # softmax prob for all u_o.T*v_c pairs
    loss = -np.log(probdist[outsideWordIdx])

    gradCenterVec = -outsideVectors[outsideWordIdx] + np.sum(outsideVectors*probdist, axis=0)

    gradOutsideVecs = (probdist*centerWordVec[:, np.newaxis].T)
    gradOutsideVecs[outsideWordIdx] = -centerWordVec*(1-probdist[outsideWordIdx])
...
def negSamplingLossAndGradient(
    ...
    simvec = (outsideVectors[indices]@centerWordVec)[:,np.newaxis] # similarity vector of o and k samples with c

    loss = float(-np.log(sigmoid(simvec[0])) - np.sum(np.log(sigmoid(-simvec[1:]))))

    ksum_vc = np.sum( outsideVectors[negSampleWordIndices]*(1-sigmoid(-simvec[1:])) , axis=0)
    gradCenterVec = (-outsideVectors[outsideWordIdx]*(1-sigmoid(simvec[0]))) + ksum_vc

    # simvec_corpus = (outsideVectors@centerWordVec)[:,np.newaxis]
    gradOutsideVecs = np.zeros(outsideVectors.shape)
    gradOutsideVecs[negSampleWordIndices] = np.outer(1-sigmoid(-simvec[1:]),centerWordVec)
    gradOutsideVecs[outsideWordIdx] = -np.outer(1-sigmoid(simvec[0]),centerWordVec)
    counts = np.bincount(indices)
    for i in np.unique(indices):
        gradOutsideVecs[i] *= counts[i] 
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    v_c = centerWordVectors[ind]
    
    for outsideword in outsideWords:
        outwordind = word2Ind[outsideword]
        l, dvc, dvw = word2vecLossAndGradient(v_c, outwordind, outsideVectors, dataset)
        loss += l
        gradCenterVecs[ind] += dvc
        gradOutsideVectors += dvw
